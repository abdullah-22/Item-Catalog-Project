"""
    ...
    The database is contain following tables:
    -- users
    -- category
    -- items
"""
...
class User(Base):
    """
        Database table to store resgistered users.
...
class Category(Base):
    """
        Database table to store categories / item types.
...
class Item(Base):
    """
        ...
        Attributes:
            ...
            user_id: User ID of the vendor of the item.
...
def db_create(DB_URL):
    """
        To create an empty database with the table defined above
    """
